...
export async function createTestimonial(c: Context) {
  const testimonial = await prisma.testimonial.create({ data: (await c.req.json()) });
  return c.json({ data: testimonial }, 201);
}

export async function updateTestimonial(c: Context) {
  const testimonial = await prisma.testimonial.update({ where: { id: c.req.param().id }, data: (await c.req.json()) });
