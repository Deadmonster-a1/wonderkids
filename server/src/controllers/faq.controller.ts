...
export async function getAllFaqs(c: Context) {
  const faqs = await c.get('prisma').faq.findMany({ orderBy: { displayOrder: 'asc' } });
  return c.json({ data: faqs });
}

export async function createFaq(c: Context) {
  const faq = await c.get('prisma').faq.create({ data: (await c.req.json()) });
  return c.json({ data: faq }, 201);
}

export async function updateFaq(c: Context) {
  const faq = await c.get('prisma').faq.update({ where: { id: c.req.param().id }, data: (await c.req.json()) });
  return c.json({ data: faq });
}

export async function deleteFaq(c: Context) {
  await c.get('prisma').faq.update({ where: { id: c.req.param().id }, data: { isActive: false } });
