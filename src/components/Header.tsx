    ...
    <>
      ...
      >
        ...
        >
          ...
          {/* Desktop Navigation */}
          <nav className="hidden md:flex items-center gap-1 bg-white/50 dark:bg-white/5 px-2 py-1.5 rounded-full border border-white/60 dark:border-white/10">
            {navLinks.map((link) => {
          ...
          <div className="flex items-center gap-3">
            <button
              onClick={toggleTheme}
              className="p-2 rounded-full bg-white/80 dark:bg-white/10 border border-slate-200 dark:border-white/10 text-brand-navy dark:text-white hover:bg-white dark:hover:bg-white/20 transition-colors"
            ...
            <Link 
              to="/admissions"
              className="hidden sm:inline-flex items-center justify-center px-6 py-2.5 rounded-full bg-brand-navy dark:bg-white text-white dark:text-brand-navy text-sm font-semibold hover:bg-brand-indigo dark:hover:bg-brand-indigo dark:hover:text-white hover:shadow-lg transition-all duration-300 hover:-translate-y-0.5"
            >
              Admissions
            </Link>
            
            <button
              className="md:hidden p-2 rounded-full bg-white/80 dark:bg-white/10 border border-slate-200 dark:border-white/10 text-brand-navy dark:text-white"
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div
            ...
            className="fixed inset-0 z-40 bg-white/90 dark:bg-[#000000]/95 pt-28 px-6"
          >
            <nav className="flex flex-col gap-4 max-w-sm mx-auto">
              {navLinks.map((link, i) => (
                ...
                >
                  <Link
                    to={link.path}
                    className="flex items-center gap-4 text-xl font-bold text-brand-navy dark:text-white p-4 rounded-2xl bg-white dark:bg-white/5 shadow-sm border border-slate-100 dark:border-white/10 hover:border-brand-indigo transition-all"
                  >
