...
export default function FaqsAdmin() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let error;
      if (isEdit) {
        const { error: updateError } = await supabase.from('Faq').update(payload).eq('id', editingItem.id);
        error = updateError;
      } else {
  ...
  return (
    <div className="bg-white dark:bg-brand-navy rounded-2xl shadow-sm border border-slate-100 dark:border-white/10 overflow-hidden">
      <div className="p-6 border-b border-slate-100 dark:border-white/10 flex justify-between items-center">
        <h2 className="text-xl font-bold text-brand-navy dark:text-white">Manage FAQs</h2>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left">
          ...
          <tbody>
            ...
            {items.length === 0 && (
              <tr>
                <td colSpan={3} className="px-6 py-8 text-center text-brand-slate">No records found.</td>
              </tr>
            )}
          </tbody>
        </table>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-brand-navy/50 backdrop-blur-sm overflow-y-auto">
          <div className="bg-white dark:bg-brand-navy rounded-2xl w-full max-w-lg shadow-xl border border-slate-100 dark:border-white/10 my-8">
            ...
            <form onSubmit={handleSave} className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-sm font-bold text-brand-navy dark:text-white mb-2">Question</label>
              ...
              <div>
                <label className="block text-sm font-bold text-brand-navy dark:text-white mb-2">Answer</label>
